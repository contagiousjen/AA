...
void alg_threads(Matrix &array3, Matrix array1, Matrix array2,
                 int number_of_threads, int size_matrix, vector<int> MulV, int part)
{
    int start, finish;
    int n_rows = size_matrix / number_of_threads;
    start = part * n_rows;
    finish = max((part + 1) * n_rows, size_matrix);
...
int main(void)
{
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        for (int i = 0; i < number_of_threads; i++)
            ...
            array2, number_of_threads, size_matrix, MulV, part++));
